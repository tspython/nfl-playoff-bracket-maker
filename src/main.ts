...
function refresh_bracket(do_set_url: boolean): void {
  ...
  if (do_set_url) {
    // Removed set_url(serial);
  }
  // Removed set_share_url(serial);
}
...
window.onpopstate = function (_e): void {
...
function select_teams(serial: number): void {
  _serial = serial;
  $.each($('.game').get().reverse(), function (_n, game) {
...
function populate_teams() {
  ...
  $('.team').each(function (_n, team) {
    const team_name = $(team).data('team');
    // const image_url = '/media/img/nfl/' + team_name + '.gif'
